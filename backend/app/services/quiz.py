...
def _local_generate_quiz(
    ...
    for card in selected:
        ...
        correct_raw = (card.detail or card.summary).split("。")[0].strip()
        correct = f"{correct_raw}。" if correct_raw else card.summary
        ...
        fallbacks = [
            f"「{card.concept}」与学科核心知识无关",
            f"「{card.concept}」的定义与知识卡片描述相反",
            "该概念仅适用于日常生活场景",
        ]
        for fb in fallbacks:
        ...
        while len(distractors) < 3:
            distractors.append(f"关于「{card.concept}」的上述说法均不正确")
